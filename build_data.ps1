$jsonContent = Get-Content "C:\Users\valen\.gemini\antigravity\scratch\raw_data_utf8.jsonl" -Raw
$jsonObj = $jsonContent | ConvertFrom-Json
$text = $jsonObj.content

$regex = '([A-Z0-9]{4,5})\s*\((Cdo\.|P\. E\.)\)\t[^\t]*\t([0-9,.]+)\t'
...
foreach ($match in $matches) {
    ...
    $priceStr = $match.Groups[3].Value
    $price = [double]($priceStr -replace '\.', '' -replace ',', '.')

    if (-not $tickers.ContainsKey($ticker) -or $tickers[$ticker].Price -lt $price) {
        $tickers[$ticker] = @{
            Ticker = $ticker
            Price = $price
        }
